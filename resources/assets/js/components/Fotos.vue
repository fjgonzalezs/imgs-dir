<template> <!-- Con el tiempo ver si es posible usar solo el componente fotos en lugar de hacer diferentes (favoritos, papelera) -->
	<div>
	...
	export default{
		...
		methods:{
			fotosGenerar(){
				 ...
				 axios.get('/carpetas/'+ this.idcarpeta +'/fotos/todas')
				.then(response => {
					this.fotos = response.data;
					 this.$Progress.finish()
					 this.cargando = false
					 this.$emit('cargado', false)
					 //console.log(response.data[0].created_at)

				});
			},
			tiempo(){
				setTimeout(()=>console.log(Date()),3000) // no hace nada
			},
			abrirModal(foto){
				this.foto = foto
